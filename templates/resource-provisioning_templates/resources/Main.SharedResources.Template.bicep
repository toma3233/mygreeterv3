...
module acr 'br/public:avm/res/container-registry/registry:0.1.1' = {
  name: '<<.sharedInput.productShortName>>-${resourcesName}-${location}acrDeploy'
  scope: resourceGroup(subscriptionId, resourceGroupName)
  params: {
...
module dataCollectionRule 'br/public:avm/res/insights/data-collection-rule:0.1.2' = {
  name: '<<.sharedInput.productShortName>>-${resourcesName}-data-collection-ruleDeploy'
  scope: resourceGroup(subscriptionId, resourceGroupName)
  params: {
    name: '<<.sharedInput.productShortName>>-${resourcesName}-data-collection-rule'
    location: rg.outputs.location
    dataFlows: [
      {
        streams: streams
...
module serviceBusNamespace 'br/public:avm/res/service-bus/namespace:0.9.0' = {
  name: '<<.sharedInput.productShortName>>-${resourcesName}-servicebus-namespaceDeploy'
  scope: resourceGroup(subscriptionId, resourceGroupName)
  params: {
    ...
    queues: [
      {
        name: '<<.sharedInput.productShortName>>-${resourcesName}-queue'
      }
    ]
    skuObject: {
      name: 'Basic'
