...
func init() {
	...
	startCmd.Flags().StringVar(&options.ServiceBusHostName, "service-bus-hostname", "<<.sharedInput.productShortName>>-resourceName-servicebus-namespace.servicebus.windows.net", "The host name used to connect to the service bus.")
	startCmd.Flags().StringVar(&options.ServiceBusQueueName, "service-bus-queue-name", "<<.sharedInput.productShortName>>-resourceName-queue", "The name of the queue to which we will send messages.")
}
