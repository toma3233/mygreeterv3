...
if [[ $clusterName =~ ^\[parameters\(.*\)\]$ ]]; then
    ...
    if [[ $parameterValue =~ ^format\(.*\)$ ]]; then
        ...
        # Example: format('servicehub-{0}-cluster', parameters('resourcesName')) -> servicehub-<value of resourcesName>-cluster
        # Value is extracted from scope bindings for example -> servicehub-official-cluster
